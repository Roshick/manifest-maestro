...
func (s *Server) setupMiddlewareStack(_ context.Context) error {
	...
	s.Router.Use(otelchi.Middleware(s.config.ApplicationName()))
	s.Router.Use(middleware.CreateLogContextCancellation(middleware.LogContextCancellationOptions{
		Description: "AddTracingToContext",
	}))

	s.Router.Use(middleware.AddTracingToContextLogger)
	s.Router.Use(middleware.CreateLogContextCancellation(middleware.LogContextCancellationOptions{
		Description: "AddTracingToContextLogger",
	}))
